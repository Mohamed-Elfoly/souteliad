...
const emptyForm = { firstName: "", lastName: "", email: "", password: "", phoneNum: "", specialty: "" };
...
export default function TeachersPanel() {
  ...
  const stats = useMemo(() => {
    ...
    return [
      { label: "إجمالي المعلمين", value: users.length, icon: "👨‍🏫", bgColor: "bg-blue-50" },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success("تم الحذف"); queryClient.invalidateQueries(["teachers-manage"]); },
    onError: (e) => toast.error(e.response?.data?.message || "خطأ في الحذف"),
  });

  const toggleMutation = useMutation({
    mutationFn: ({ id, active }) => updateUser(id, { active }),
    onSuccess: () => { toast.success("تم تحديث الحالة"); queryClient.invalidateQueries(["teachers-manage"]); },
  ...
  const openEdit = (user) => {
    setEditingUser(user);
    setForm({ firstName: user.firstName, lastName: user.lastName, email: user.email, phoneNum: user.phoneNum || "", specialty: user.specialty || "" });
    setShowModal(true);
  };
  ...
  const bulkDelete = async () => {
    if (!window.confirm(`حذف ${selectedIds.length} مستخدم؟`)) return;
  ...
  return (
    <div className="space-y-5">
      ...
      <UserFormModal
        ...
        title={editingUser ? "تعديل بيانات المعلم" : "إضافة معلم جديد"}
        form={form}
        setForm={setForm}
