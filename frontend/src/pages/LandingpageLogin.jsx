...
import LevelsSection from "../components/landingPage/LevelsSection";
...
export default function LandingpageLogin() {
  ...
  const completedIds = new Set(
    (progressData?.data?.data?.data || []).map(
      (p) => p.lessonId?._id || p.lessonId
    )
  );
  ...
  const lessons = [...(lessonsData?.data?.data?.data || [])].sort(
  ...
  return (
    <div className="lp-container">
      ...
      <LevelsSection
        sortedLevels={sortedLevels}
        effectiveLevelId={effectiveLevelId}
        setActiveLevelId={setActiveLevelId}
        lessons={lessons}
        lessonsLoading={lessonsLoading}
        completedIds={completedIds}
        levelImages={LEVEL_IMAGES}
        fallbackThumbs={FALLBACK_THUMBS}
      />
